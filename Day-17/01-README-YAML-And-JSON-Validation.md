...
**Never use `yaml.load(f)` without an explicit `Loader`, and never use `yaml.unsafe_load()` / `Loader=yaml.UnsafeLoader` on any file you don't fully control.** PyYAML's non-safe loaders honor tags like `!!python/object` that construct arbitrary Python objects during parsing — this is a deserialization vulnerability in the same class as unpickling untrusted data (CWE-502): a crafted YAML file (a pulled-in Helm chart, a user-submitted config in a multi-tenant tool) can execute code the moment it's loaded. `yaml.safe_load()` restricts construction to plain built-in types and is what you should reach for by default — for trusted and untrusted YAML alike, because a config file legitimately never needs to construct an arbitrary Python object graph.
...
- `yaml.safe_load()` is the default choice always. `yaml.load()` without a `Loader`, or `yaml.unsafe_load()`, can construct arbitrary Python objects from the file (a CWE-502-class deserialization risk, not a theoretical one).
...
- Using `yaml.load(f)` (no `Loader` arg) or `yaml.unsafe_load()`/`Loader=yaml.UnsafeLoader` on config that could ever originate outside your own commits — a real code-execution vector, not a hypothetical one.
